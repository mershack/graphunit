setwd("/disk/vizlab/apps/apache-tomcat-7.0.52/webapps/graphunit/studies/study50C/data")
sink("wilcoxon-rank-sum-analysis.txt")
accuracy1 = read.csv("AccuracyResults1.txt")
accuracy2 = read.csv("AccuracyResults2.txt")
time1 = read.csv("TimeResults1.txt")
time2 = read.csv("TimeResults2.txt")
taskname="TaskName = Acc_selectNodeWithHighestDegree2"
cat(taskname)
cat(paste("\nTaskname ," ,"p-value"))
cat("\n----------------------------------------------------")
Acc_selectNodeWithHighestDegree2_NL= c(accuracy1[,1])
Acc_selectNodeWithHighestDegree2_MAT= c(accuracy2[,1])
result = wilcox.test(Acc_selectNodeWithHighestDegree2_NL,Acc_selectNodeWithHighestDegree2_MAT)
cat(paste("\nAcc_selectNodeWithHighestDegree2", " , " , result$p.value, "\n" ))
Acc_selectMoreInterconnectedCluster2Clusters_NL= c(accuracy1[,2])
Acc_selectMoreInterconnectedCluster2Clusters_MAT= c(accuracy2[,2])
result = wilcox.test(Acc_selectMoreInterconnectedCluster2Clusters_NL,Acc_selectMoreInterconnectedCluster2Clusters_MAT)
cat(paste("\nAcc_selectMoreInterconnectedCluster2Clusters", " , " , result$p.value, "\n" ))
Acc_selectAllNeigbhorsOf1Node_NL= c(accuracy1[,3])
Acc_selectAllNeigbhorsOf1Node_MAT= c(accuracy2[,3])
result = wilcox.test(Acc_selectAllNeigbhorsOf1Node_NL,Acc_selectAllNeigbhorsOf1Node_MAT)
cat(paste("\nAcc_selectAllNeigbhorsOf1Node", " , " , result$p.value, "\n" ))
cat("\n\n")
taskname="TaskName = Time_selectNodeWithHighestDegree2"
Time_selectNodeWithHighestDegree2_NL= c(time1[,1])
Time_selectNodeWithHighestDegree2_MAT= c(time2[,1])
result = wilcox.test(Time_selectNodeWithHighestDegree2_NL,Time_selectNodeWithHighestDegree2_MAT)
cat(paste("\nTime_selectNodeWithHighestDegree2", " , " , result$p.value, "\n" ))
Time_selectMoreInterconnectedCluster2Clusters_NL= c(time1[,2])
Time_selectMoreInterconnectedCluster2Clusters_MAT= c(time2[,2])
result = wilcox.test(Time_selectMoreInterconnectedCluster2Clusters_NL,Time_selectMoreInterconnectedCluster2Clusters_MAT)
cat(paste("\nTime_selectMoreInterconnectedCluster2Clusters", " , " , result$p.value, "\n" ))
Time_selectAllNeigbhorsOf1Node_NL= c(time1[,3])
Time_selectAllNeigbhorsOf1Node_MAT= c(time2[,3])
result = wilcox.test(Time_selectAllNeigbhorsOf1Node_NL,Time_selectAllNeigbhorsOf1Node_MAT)
cat(paste("\nTime_selectAllNeigbhorsOf1Node", " , " , result$p.value, "\n" ))
cat("\n\n")
sink()
